...
class VqaJobAdmin(admin.ModelAdmin):
    list_display = ('job_id', 'image_url', 'input_answer', 'predicted_answer', 'gcam_image_url', 'createdAt')

    def image_url(self, obj):
        return '<a target="_blank" href="%s">%s</a>' % (obj.image, obj.image)
    image_url.allow_tags = True


    def gcam_image_url(self, obj):
        return '<a target="_blank" href="%s">%s</a>' % (obj.gcam_image, obj.gcam_image)
